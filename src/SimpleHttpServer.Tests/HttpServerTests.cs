...
public class HttpServerTests(ITestOutputHelper output)
{
    ...
    public async Task SendRequests()
    {
        ...
        var url = $"http://localhost:{port}/{Guid.NewGuid()}/{Guid.NewGuid()}?query=123";
        ...
        using var server = new HttpServer(port, handler);
        server.Start();
    ...
    private class RequestHandler(
        ...
        Action<Exception> errorHandler) : IHttpRequestHandler
    {
        public HttpResponse Handle(HttpRequest request) => handler(request);

        public void OnError(Exception exception) => errorHandler(exception);
    }
