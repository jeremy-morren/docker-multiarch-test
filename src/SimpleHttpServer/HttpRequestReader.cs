...
public static class HttpRequestReader
{
    public static HttpRequest? Read(Stream stream)
    {
        ...
        if (headers.TryGetValue("Content-Length", out var clHeader))
        {
            ...
            if (!int.TryParse(clHeader, out var length) || length < 0)
        ...
        else if (headers.TryGetValue("Transfer-Encoding", out var teHeader)
            && teHeader.Equals("chunked", StringComparison.OrdinalIgnoreCase))
        {
    ...
    private static Uri GetUrl(string path, Dictionary<string, string> headers)
    {
